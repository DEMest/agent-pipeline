...
коммитов, ждёт первого коммита: раньше проверять нечего. Поэтому она вынесена в Шаг 5 — сразу
после первого коммита и до `git push`.
...
2. Включены ли Actions в репозитории:
   ...
   репозитория.
3. Атрибуция коммитов — проверяется в Шаге 5, сразу после первого коммита.
...
Атрибуция коммитов — сразу после первого коммита и до `git push`: выполнить
`gh api repos/<owner>/<repo>/commits --jq '.[0].author.login'`. Пусто или чужой аккаунт —
предложить `git config --local user.email` с адресом `<id>+<login>@users.noreply.github.com`,
где id берётся из `gh api user`, и переписать уже сделанный коммит
(`git commit --amend --reset-author --no-edit`), пока он ещё не ушёл в `origin`. Именно здесь,
а не в Шаге 1: раньше коммита не существует и проверять нечего, а после `git push` коммит с чужим
авторством уже уедет в удалённый репозиторий.
